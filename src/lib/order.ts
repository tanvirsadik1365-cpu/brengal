import { baseCurry, menuSections } from "@/lib/restaurant";
...
export const ONION_BHAJI_THRESHOLD = 30;
export const SIDE_DISH_THRESHOLD = 45;
...
export function getCatalogItems(): CatalogItem[] {
  const baseItems = baseCurry.prices.map((item) => ({
    id: `main-${item.name}`,
...
export function getActiveReward(subtotal: number, orderType: OrderType): ActiveReward {
  if (subtotal >= COMBO_THRESHOLD) {
    return {
      type: "combo",
      title: "Free Onion Bhaji + Side Dish",
      detail: "Choose one side dish. Onion Bhaji is included automatically.",
      requiresSideDish: true,
    };
  }

  if (subtotal >= SIDE_DISH_THRESHOLD) {
    return {
      type: "side-dish",
      title: "Free Side Dish",
      detail: "Choose one side dish on the cart page.",
      requiresSideDish: true,
    };
  }

  if (subtotal >= ONION_BHAJI_THRESHOLD) {
    return {
      type: "onion-bhaji",
      title: "Free Onion Bhaji",
      detail: "Onion Bhaji is included automatically.",
  ...
  return {
    ...
    detail:
      orderType === "delivery"
        ? `${formatCurrency(Math.max(DELIVERY_MINIMUM - subtotal, 0))} more for delivery.`
